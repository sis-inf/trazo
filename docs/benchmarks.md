...
Los tiempos medidos son promedios de 5 ejecuciones. Los métodos se ejecutaron con diferentes tamaños de entrada para obtener una medida robusta.
...
Los resultados muestran que los métodos más rápidos son `sqrt` y `add`, mientras que `power` y `divide` son los más lentos en este escenario.